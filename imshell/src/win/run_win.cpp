// Win32 host loop, extracted from main.cpp (code-contract §11): the
// framework-mode WinMain is a thin wrapper over run(); a library
// consumer calls run() from their own main/WinMain (the subsystem is
// the consumer executable's link-time choice). The loop protocol, the
// I-2a presentation mapping, and the A-2 seams are unchanged.
#include <windows.h>
#include <windowsx.h>
#include <string>
...
namespace zb::shell
{
    int run(zb::SharedPtr<zb::app::IApp> app, const run_options &options)
    {
        ...
        if (!window_class_registered)
        {
            ...
            {
                throw zb::ui::error("Win32 shell: RegisterClassEx failed");
            }
        ...
        if (g_app->window() == nullptr)
        {
            ...
            {
                g_app->create_window(options.width, options.height);
            }
            else
            {
                g_app->create_window();
        ...
        if (nullptr == hwnd)
        {
            throw zb::ui::error("Win32 shell: CreateWindowEx failed");
        }
