...
//
// The host loop is extracted into zb::shell::run (code-contract §11):
// the framework-mode main.mm is a thin wrapper; a library consumer
// calls run() from their own main (OBJCXX must be enabled in the
// consumer, the imprint top-level CMake does it under
// add_subdirectory).
//
...
namespace zb::shell
{
    int run(zb::SharedPtr<zb::app::IApp> app, const run_options &options)
    {
        ...
        {
            throw zb::ui::error("the mac shell presents the 32bpp internal layout only");
        }
        ...
        @autoreleasepool
        {
            ...
            win.title = [NSString stringWithUTF8String:os_title.c_str()];
