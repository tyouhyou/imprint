// X11 host loop, extracted from main_x.cpp (code-contract §11): the
// framework-mode main is a thin wrapper over run(); a library consumer
// calls run() from their own main. The loop protocol, the I-2a
// presentation mapping, and the A-2 seams are unchanged.
#include <vector>
#include <X11/Xlib.h>
#include <X11/Xutil.h>
#include <X11/Xatom.h>
...
namespace zb::shell
{
    int run(zb::SharedPtr<zb::app::IApp> app, const run_options &options)
    {
        ...
        if (app->window() == nullptr)
        {
            ...
            {
                app->create_window(options.width, options.height);
            }
            else
            {
                app->create_window();
        ...
        const std::string os_title =
            !options.title.empty() ? options.title : win->title();
        ...
        // never crop anything
        XStoreName(display, window, os_title.c_str());
