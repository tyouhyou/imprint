...
| `imui`    | Widget tree: `Panel`, `FlexPanel`, `Button`, `Checkbox`, `RadioButton`, `Label`, `Slider`, `ProgressBar`, `ListBox`, `TextInput`, `Dialog`, `ToggleSwitch`, `GaugeDial`, `Knob`, `TrendLine` (V-5 composition widgets), `SvgCanvas` (HTML `svg`/`vectordial` vector-dial subset), `InputDispatcher`; design-file layer (`ui_builder`, `ui_file`, `html`) | STATIC | `imcore`, `imevent`, `iminput`      |
...
| `imapp_snapshot` | Optional snapshot helper (`zb::snap`, header-only): frame hash + baseline record/check + GIF/PNG dumps — the deterministic-test story (code-contract §12) | INTERFACE | `imapp` |
...
- **`on_input()` override pattern (A-24).** `on_input()` is the sole event
  ...
  **not** claimed, so no lock forms). A widget that wants drag semantics
  ...
  receives moves at all — a move still over the widget or its subtree (a
  container that consumed the press owns its children's area), or off both
  but within the slop, only resets the drift counter; a move off the target
  beyond the slop cancels the press. The full author-facing sequence (8 px
  slop, the two-consecutive-strike touch debounce with its reset rule, and
  release-always-reaches) is specified once in code-contract §3.3's
  pointer rules; this section pins only the shape. A release always
  reaches the held
...
- `ui_node` is the single intermediate representation (fluent builders and
  the text parsers both produce it); `build(host, root)` materializes it into
  a live widget tree; `parse_ui_text` parses the minimal text format and
  `parse_html` the HTML/CSS subset (`docs/html-path.md`) — the two front-ends
