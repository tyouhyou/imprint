...
| `imui`    | Widget tree: `Panel`, `FlexPanel`, `Button`, `Checkbox`, `RadioButton`, `Label`, `Slider`, `ProgressBar`, `ListBox`, `TextInput`, `Dialog`, `ToggleSwitch`, `GaugeDial`, `Knob`, `TrendLine` (V-5 composition widgets), `InputDispatcher`; design-file layer (`ui_builder`, `ui_file`) | STATIC | `imcore`, `imevent`, `iminput`      |
